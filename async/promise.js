...
Promise.resolve('promise done').then((data) => { console.log(data)}); // microtask queue, event loop consider microtask with higher prriority
Promise.resolve('promise2 done').then((data) => { for(let i=0; i<100000000; i++){}console.log(data)}); // microtask queue, event loop consider microtask with higher prriority
...
lotteryPromise.then(data=>console.log(data)).catch(err=>console.log(err.message));
...
wait(2)
.then(data => { console.log(data); return wait(1)}) // Return prmoise has to be there for chaining
...
(async function() {
  console.log('starting');
  const res = await Promise.race([  // FIRST RESPONSE WINS
    fetch('https://jsonplaceholder.typicode.com/users'),
    fetch('https://jsonplaceholder.typicode.com/posts'),
    fetch('https://profile-app-mw.herokuapp.com/user/comment')
    ]);
